...
type
  ...
  TMangoHudSettings = record
    ...
    Versioning: Boolean;
    AutoUpload: Boolean;
    // Preset state — when True the config file contains only the fps_only token
    FpsOnly: Boolean;
  end;
...
begin
  ...
  ConfigLines := TStringList.Create;
  try
    // ── fps_only preset shortcut ─────────────────────────────────────────────
    // When the fps_only token was loaded from the config file (or the user just
    // clicked the "FPS Only" preset), write the single-token file and mirror it
    // to the system-wide locations — identical to what the normal save does —
    // then return immediately without generating a full config from UI state.
    if Settings.FpsOnly then
    begin
      ConfigLines.Add('fps_only');
      ConfigLines.SaveToFile(Settings.MangoHudCfgFile);

      // Update bgmod.conf so MANGOHUD_CONFIGFILE stays in sync
      FGModFilePath := GetGameConfigDir(Settings.ActiveGameName) + 'bgmod.conf';
      ForceDirectories(ExtractFilePath(FGModFilePath));
      Ini := TIniFile.Create(FGModFilePath);
      try
      ...
      // Mirror to ~/.config/MangoHud/MangoHud.conf in global mode
      if Settings.ActiveGameName = '' then
      begin
      ...
      // Mirror to Steam Flatpak location
      try
        ...
        begin
          ...
          begin
            ...
            WriteLn('[DEBUG] SaveMangoHudConfigCore: fps_only also mirrored to Steam Flatpak: ', FlatpakMangoHudFile);
          end;
        end;
      except
        on E: Exception do
          WriteLn('[WARN] SaveMangoHudConfigCore: Could not mirror fps_only to Steam Flatpak: ', E.Message);
      end;

      Result := True;
      Exit; // ← skip full config generation
    end;
    // ── end fps_only shortcut ─────────────────────────────────────────────────
