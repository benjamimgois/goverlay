...
void main() {
    ...
    vec3 shadowColor = pow(vec3(0.06, 0.06, 0.07), vec3(2.2));    // soft dark gray/black shadow
    ...
    vec2 diff = (uv - shadowCenter) * vec2(1.7777778, 1.0) / (vec2(0.40, 0.065) * scaleFactor);
    float shadowDist = length(diff);
    float shadowIntensity = pow(1.0 - smoothstep(0.0, 1.0, shadowDist), 1.5);
    
    // Apply soft drop shadow (ambient occlusion on the invisible studio floor)
    color = mix(color, shadowColor, shadowIntensity * 0.55);
