...
{FillChar(ImageMemoryBarrier,SizeOf(TVkImageMemoryBarrier),#0);
 ...
 for MipMapLevelIndex:=0 to fInstance.ImageBasedLightingReflectionProbeCubeMaps.MipMaps-1 do begin
  ...
  if assigned(fInstance.Renderer.VulkanDevice.BreadcrumbBuffer) then begin
   fInstance.Renderer.VulkanDevice.BreadcrumbBuffer.BeginBreadcrumb(aCommandBuffer.Handle,TpvVulkanBreadcrumbType.Dispatch,'ReflectionProbeMipMapComputePass');
  end;
