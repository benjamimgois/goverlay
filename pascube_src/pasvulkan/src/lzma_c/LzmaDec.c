...
// Disable replacing with memcpy() for some compilers
static void local_memcpy(void *dest, const void *src, long long int count)
{
  char *d = (char *)dest;
  const char *s = (const char *)src;
  while (count >= sizeof(long long int)){
    *(long long int *)d = *(long long int *)s;
    d += sizeof(long long int);
    s += sizeof(long long int);
    count -= sizeof(long long int);
  }
...
int32_t CALLCONV LzmaDec_Size(){
  return sizeof(CLzmaDec);
}

void CALLCONV LzmaDec_Init(CLzmaDec *p)
{
  p->dicPos = 0;
...
SRes CALLCONV LzmaDec_DecodeToDic(CLzmaDec *p, SizeT dicLimit, const Byte *src, SizeT *srcLen,
...
SRes CALLCONV LzmaDec_DecodeToBuf(CLzmaDec *p, Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen, ELzmaFinishMode finishMode, ELzmaStatus *status)
{
...
void CALLCONV LzmaDec_FreeProbs(CLzmaDec *p, ISzAllocPtr alloc)
{
...
void CALLCONV LzmaDec_Free(CLzmaDec *p, ISzAllocPtr alloc)
{
  LzmaDec_FreeProbs(p, alloc);
  LzmaDec_FreeDict(p, alloc);
}

SRes CALLCONV LzmaProps_Decode(CLzmaProps *p, const Byte *data, unsigned size)
{
  UInt32 dicSize;
  Byte d;
...
SRes CALLCONV LzmaDec_AllocateProbs(CLzmaDec *p, const Byte *props, unsigned propsSize, ISzAllocPtr alloc)
{
  CLzmaProps propNew;
...
SRes CALLCONV LzmaDec_Allocate(CLzmaDec *p, const Byte *props, unsigned propsSize, ISzAllocPtr alloc)
{
  CLzmaProps propNew;
  SizeT dicBufSize;
  RINOK(LzmaProps_Decode(&propNew, props, propsSize))
...
SRes CALLCONV LzmaDecode(Byte *dest, SizeT *destLen, const Byte *src, SizeT *srcLen,
