...
# -*- coding: utf-8 -*-
"""
q42nl_testcase.py — self-contained tests for Q42NL quantization
- Encodes random tensors with q42nl
- Verifies byte size/layout (32 vals -> 18 bytes)
- Decodes back and reports MSE/PSNR
- Benchmarks grid sizes and optional chunking
Usage:
  python q42nl_testcase.py                # quick small test
  python q42nl_testcase.py --cuda         # run on GPU if available
  python q42nl_testcase.py --shape 65536 32
  python q42nl_testcase.py --grid 33
  python q42nl_testcase.py --chunk 262144 # process elements per chunk (multiple of 32)
"""
import os
import time
import math
...
def q42nl(
    ...
    """
    Quantize a tensor using the Q42NL method.
    """
...
def q42nl_dequant(packed: torch.Tensor, device=None) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Unpack q42nl bytes -> (q:int8 [G,32], scale:float32 [G], curve:float32 [G])
    Returns tensors on 'device' (defaults to packed.device).
    """
    if device is None:
        device = packed.device
    packed = packed.view(-1, 18).to(torch.uint8)                  # [G,18]
    G = packed.shape[0]
    ...
    scale = sbyte.view(torch.float8_e5m2).to(torch.float32)       # [G]
    curve = (cbyte.view(torch.int8).to(torch.float32) / 127.0).clamp(-1.0, 1.0)  # [G]

    return q, scale, curve

def q42nl_dequant_reconstruct(packed: torch.Tensor, device=None) -> torch.Tensor:
    """Full reconstruction to float32 values using stored q, scale, curve."""
    q, scale, curve = q42nl_dequant(packed, device=device)        # [G,32], [G], [G]
...
def main():
    ...
    if args.chunk and args.chunk > 0:
        packed = q42nl_chunked(t_flat, chunk_elems=args.chunk, grid_size=args.grid)
    else:
        packed = q42nl(t_flat, grid_size=args.grid)
    torch.cuda.synchronize() if device.type == "cuda" else None
    t1 = time.time()

    # checks
    expected_bytes = (numel // 32) * 18
    ...
    # reconstruct
    t2 = time.time()
    recon = q42nl_dequant_reconstruct(packed, device=device)
    torch.cuda.synchronize() if device.type == "cuda" else None
    t3 = time.time()
    ...
    q, scale, curve = q42nl_dequant(packed, device=device)
