...
* *Parameters:*
** `+callback+` (function): A function that tests each element of the
array. Return `+true+` to keep the element, `+false+` otherwise. It
...
// Filter for values greater than 3
let greaterThanThree = numbers.filter(function(x) { return x > 3; });
// greaterThanThree is [4, 5, 6]
----
