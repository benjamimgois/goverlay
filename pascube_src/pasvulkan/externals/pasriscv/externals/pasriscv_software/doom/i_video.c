...
void I_FinishUpdate(void)
{
    ...
    const int channel_size = 4;//DOOMGENERIC_CHANNELS;
    ...
    for (int y = 0; y < y_size; y++)
    {
        ...
        for (int x = 0; x < x_size; x++)
        {
            ...
            uint32_t* fb = (uint32_t*)DOOMGENERIC_FB;
            fb += i_x_colors;// * channel_size;
            ...
            fb[2] = c->b;*/
            //*(struct color*)fb = *c;
            *fb = 0xff000000u | (((uint32_t)c->b) << 16u) | (((uint32_t)c->g) << 8u) | (((uint32_t)c->r) << 0u);
        }
    }
    //printf("!2\n");
    DG_DrawFrame();
