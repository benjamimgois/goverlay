...
set -x
exec > >(tee -i /tmp/bgmod-uninstaller.log) 2>&1
...
error_exit() {
  ...
  logger -t bgmod-uninstaller "❌ ERROR: $1"
  exit 1
}
...
logger -t bgmod-uninstaller "🟢 Uninstalling from: $exe_folder_path"
...
# === Remove Config Files Installed by bgmod ===
echo "🧹 Removing config files installed by bgmod..."
rm -f "MangoHud.conf" "vkBasalt.conf" "vkSumi.conf" "bgmod.conf" "bgmod.log"
...
for dll in "${original_dlls[@]}" "${proxy_dlls[@]}"; do
  if [[ -f "${dll}.b" ]]; then
    mv -f "${dll}.b" "$dll"
    echo "✅ Restored original $dll"
    logger -t bgmod-uninstaller "✅ Restored original $dll"
  fi
done

# === Self-remove uninstaller ===
echo "🗑️ Removing uninstaller..."
rm -f "bgmod-uninstaller.sh"

echo "✅ bgmod removed from this game successfully!"
logger -t bgmod-uninstaller "✅ bgmod removed from $exe_folder_path"
