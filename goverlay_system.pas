...
begin
  ...
  begin
    ...
    // vkSumi extension or user-space layer
  ...
  begin
    ...
    if not FileExists('/usr/share/vulkan/implicit_layer.d/vksumi.json') and
       ...
       not ((GetEnvironmentVariable('XDG_DATA_HOME') <> '') and FileExists(IncludeTrailingPathDelimiter(GetEnvironmentVariable('XDG_DATA_HOME')) + 'vulkan/implicit_layer.d/vksumi.json')) and
       not FileExists(GetUserDir + '.local/share/goverlay/layers/vksumi/libVkLayer_vksumi.so') and
       not ((GetEnvironmentVariable('XDG_DATA_HOME') <> '') and FileExists(IncludeTrailingPathDelimiter(GetEnvironmentVariable('XDG_DATA_HOME')) + 'goverlay/layers/vksumi/libVkLayer_vksumi.so')) and
    ...
    // MAKO / lsfg-vk: check Vulkan layer JSON then fall back to library scan or binary
