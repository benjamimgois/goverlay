...
uses
  xlib, x, tweaks_md3, games_tab, vkbasalt_tab, mangohud_ui;
...
begin
  ...
  begin
    if ErrMsg <> '' then
      ShowStatusMessage('Error saving MangoHud config: ' + ErrMsg);
  end;
end;
...
begin
  ...
  FHomeModDots[3].Brush.Color := IfThen(SumiOK,  CLR_OK, CLR_MISSING);
  ...
  SumiVer := GetVkSumiVersion;
  if SumiVer = '' then SumiVer := IfThen(SumiOK, 'installed', 'not found');
