...
type
  ...
  Tgoverlayform = class(TForm)
   ...
   // hudbackgroundColorButton: TColorButton;
    ...
    procedure mangohudPanelMouseEnter(Sender: TObject);
...
var
  ...
  initpcidevsave: Textfile;
  initpcidevsaveSTR: string;
...
begin
  ...
  case pcidevCombobox.ItemIndex of
    //Add string "0:" before actual pcidev number and save device position used in goverlay folder
    0:RunCommand('bash -c ''echo -n "pci_dev=0:" >> $HOME/.config/MangoHud/MangoHud.conf | cat /tmp/goverlay/pcidev/pcidev1 >> $HOME/.config/MangoHud/MangoHud.conf | echo "pcidev1" >> $HOME/.config/goverlay/pcidev_save''', s);
    1:RunCommand('bash -c ''echo -n "pci_dev=0:" >> $HOME/.config/MangoHud/MangoHud.conf | cat /tmp/goverlay/pcidev/pcidev2 >> $HOME/.config/MangoHud/MangoHud.conf | echo "pcidev2" >> $HOME/.config/goverlay/pcidev_save ''', s);
    2:RunCommand('bash -c ''echo -n "pci_dev=0:" >> $HOME/.config/MangoHud/MangoHud.conf | cat /tmp/goverlay/pcidev/pcidev3 >> $HOME/.config/MangoHud/MangoHud.conf | echo "pcidev3" >> $HOME/.config/goverlay/pcidev_save''', s);
    3:RunCommand('bash -c ''echo -n "pci_dev=0:" >> $HOME/.config/MangoHud/MangoHud.conf | cat /tmp/goverlay/pcidev/pcidev4 >> $HOME/.config/MangoHud/MangoHud.conf | echo "pcidev4" >> $HOME/.config/goverlay/pcidev_save''', s);
  end;
...
procedure Tgoverlayform.mangohudPanelMouseEnter(Sender: TObject);
begin
...
begin
  ...
  RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "Classic Mode" "This mode is a workaround for GTK desktops with no Breeze Theme"''', s); // Popup a notification
...
begin
  case replaygeSpeedButton.imageIndex of
    0: begin
      ...
      RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "ReplaySorcery Global Enable Activated" "The desktop will be continually recorded in the background"''', s); // Popup a notification
...
begin
  ...
  // Popup a notification
  RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "ReplaySorcery" "Configuration saved"''', s);
  ...
  case audiosampCombobox.ItemIndex of
    ...
    4:begin
      ...
      //REPLAY Save message
      //RunCommand('bash -c ''echo "preOutputCommand = notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png Saving_video_recording" >> $HOME/.config/replay-sorcery.conf''', s);
      RunCommand('bash -c ''echo "outputCommand = notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png Video_Saved_in_the_home_folder" >> $HOME/.config/replay-sorcery.conf''', s);
...
begin
   RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "Replay-Sorcery" "Service running in background, press the toggle key clip it"''', s);
...
begin
   RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "Replay-Sorcery" "Service stopped"''', s);
...
begin
  ...
  RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "Syncing Reshade repositories..." "This can take a few seconds, please wait until complete" | git clone https://github.com/benjamimgois/reshade-shaders.git $HOME/.local/share/reshade''', s);
  RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "Sync Complete"''', s); // Popup a notification
...
//#########################################       RESHADE SHADERS       ###########################################################################
  ...
  // Popup a notification
  RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "vkBasalt" "Configuration saved"''', s);
...
begin
     case basaltgeSpeedButton.imageIndex of
       ...
       RunCommand('bash -c ''notify-send -e -i /usr/share/icons/hicolor/128x128/apps/goverlay.png "vkBasalt Global Enable Activated" "Every Vulkan application will have these effects applied"''', s); // Popup a notification
       showmessage ('Restart your system to take effect');
     end;
     1: begin
...
RunCommand('bash -c ''touch /tmp/goverlay/pcidev/pcidev_save''', s);
...
RunCommand('bash -c ''yes | cp -rf $HOME/.config/goverlay/pcidev_save /tmp/goverlay/pcidev/''', s);
...
// Store pcidev number of third line and GPU description in separated files
...
// Store pcidev number of fourth line and GPU description in separated files
...
Readln(initpcidevsave,initpcidevsaveSTR); //Assign Text file to String
CloseFile(initpcidevsave);
