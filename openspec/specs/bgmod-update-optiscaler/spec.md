...
The `bgmod` execution wrapper SHALL copy `OptiScaler.ini` and `fakenvapi.ini` from GOverlay's config directory to the game directory on every run when OptiScaler is enabled (`GOverlayOptiscaler = True`).

For `OptiScaler.ini`, `bgmod` SHALL copy `OptiScaler.ini` from `ConfigDir` into `GameDir` whenever `ConfigDir/OptiScaler.ini` exists, and MUST NOT overwrite `GameDir/OptiScaler.ini` with pristine cache templates prior to syncing.

For `fakenvapi.ini`, `bgmod` SHALL copy `fakenvapi.ini` from `ConfigDir` into `GameDir` whenever `ConfigDir/fakenvapi.ini` exists.

#### Scenario: OptiScaler.ini synced on launch
- **WHEN** `bgmod` launches a game with OptiScaler enabled and `ConfigDir/OptiScaler.ini` exists
- **THEN** `bgmod` copies `OptiScaler.ini` from `ConfigDir` into `GameDir`.

#### Scenario: fakenvapi.ini synced on launch
- **WHEN** `bgmod` launches a game with OptiScaler enabled and `ConfigDir/fakenvapi.ini` exists
- **THEN** `bgmod` copies `fakenvapi.ini` from `ConfigDir` into `GameDir`.
