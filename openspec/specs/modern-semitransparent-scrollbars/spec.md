...
GOverlay SHALL style scrollbars across all scrollable controls (`TScrollBox`, `TMemo`, `TListBox`, `TListView`, `TScrollBar`) using narrow (6px), semitransparent QSS rules.

#### Scenario: Scrolling in any tab or control
- **WHEN** user interacts with or views a scrollable container in GOverlay
- **THEN** the scrollbar appears as a narrow (6px) semitransparent rounded pill without arrow buttons, highlighting smoothly on hover.
