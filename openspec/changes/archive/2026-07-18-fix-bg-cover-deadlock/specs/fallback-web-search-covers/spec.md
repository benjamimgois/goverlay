...
- **THEN** GOverlay SHALL thread-safely generate/load on the main GUI thread a fallback cover image containing the GOverlay icon centered on a dark background, save it to the cache path, and update the game card image in the UI.
