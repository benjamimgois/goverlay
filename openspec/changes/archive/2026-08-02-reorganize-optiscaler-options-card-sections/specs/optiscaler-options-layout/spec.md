...
The system SHALL place the `File name` combobox, `Scale` trackbar with resolution markers (1080p, 1440p, 4K), `OptiPatcher` checkbox, and `Menu Toggle Key` shortcut capture button within the "Main" sub-section.

#### Scenario: Interacting with Main sub-section controls
- **WHEN** the user modifies File name, Scale slider, OptiPatcher checkbox, or Menu Toggle Key
- **THEN** the corresponding OptiScaler configuration values are updated and saved accordingly
